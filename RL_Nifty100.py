...
data = pd.read_csv('Nifty100_3yrs.csv')
...
data['RSI'] = calculate_rsi(data, window=15)
...
testing_data = pd.read_csv('Nifty100.csv')
...
trained_model = PPO.load("stock_trading_agent")
# Create an instance of the testing environment
testing_env = StockTradingEnvironment(testing_data)
...
portfolio_values = [testing_data.iloc[0]['Close']]  # Initial portfolio value

while not done:
    action, _ = trained_model.predict(obs)
    obs, _, done, _ = testing_env.step(action)
    portfolio_values.append(testing_data.iloc[testing_env.current_step]['Close'] * (1 + obs[0]))
...
# Calculate annualized mean return and risk-free rate (assumed as 0.02 for example)
